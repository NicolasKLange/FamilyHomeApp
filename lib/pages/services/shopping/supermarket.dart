...
class _SupermarketState extends State<Supermarket> {
  ...
  final String category = "Mercado"; // Define a categoria
  ...
  void getOnTheLoad() async {
    ...
    if (familyId != null) {
      setState(() {
        todoStream = _databaseMethods.getProducts(familyId, "Mercado");
      });
    }
  }
  ...
  void deleteShoppingList() async {
    String? familyId = await _databaseMethods.getFamilyId();
    if (familyId != null) {
      await _databaseMethods.deleteProductList(familyId, "Mercado");
      setState(() {
